...
const findAntiNodeCoordinates = (nodeCoordinates, maxWidth, maxHeight) => {
    const anc = [];
    for (var i = 0; i < nodeCoordinates.length - 1; i++) {
        for (var j = i + 1; j < nodeCoordinates.length; j++) {
            ...
            const toRepeat = Math.max(
                Math.ceil(maxWidth / Math.abs(deltaX)),
                Math.ceil(maxHeight / Math.abs(deltaY))
            );

            for (var k = 0; k <= toRepeat; k++) {
                anc.push({
                    x: node1.x - (k * deltaX),
                    y: node1.y - (k * deltaY)
                })
...
export default function (data, { isTest }) {
    ...
    const collection = keys.map(key => ({
        key,
        nodes: map[key],
        antiNodes: findAntiNodeCoordinates(map[key], data[0].length, data.length)
    }));
